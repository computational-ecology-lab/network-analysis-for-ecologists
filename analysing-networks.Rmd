...
# 1 Methods for network analysis
...
This, and other ecological networks datasets, are available from the [Global Food Web Database](https://www.canberra.edu.au/globalwebdb/).
...
benguela.EL <- benguela.EL + 1  # Because the species indices start from 0 on the file
```
...
If we have an adjacency matrix and we want an igraph object representing the same network, without the need to create the network from scratch, igraph offers the functionality of creating graphs from adjacency matrices. Just use the function `graph_from_adjacency_matrix(A)`, where A is your adjacency matrix.
...
***Generality***
$$G_i = \frac{1}{L/S}\sum_{j=1}^{S}a_{ji}$$
***Vulnerability***
$$V_i = \frac{1}{L/S}\sum_{j=1}^{S}a_{ij}$$
...
print(MeanFoodChainLength(benguela.AM))
...
### 1.6.9 Centrality and Clustering

Different centrality measures exist. Here we will focus on **betweenness centrality**, which measures how frequently a node appears on the shortest path between other nodes in the graph. This is a node-level metric that can be used to rank nodes according to their importance in terms of how central they are in the network.

```{r}
require(ggplot2)
# Betweenness centrality using igraph
between <- igraph::betweenness(benguela.network)

between_df <- data.frame(vertex=as.numeric(V(benguela.network)), between)
between_df <- between_df[order(between_df$between, decreasing = TRUE),]
between_df$rank <- 1:dim(between_df)[1]

ggplot(between_df, aes(rank, between)) + geom_point(colour='orange', size=3) + theme_bw() +
  xlab('Rank') + ylab('Betweenness Centrality') + 
  geom_text(aes(label=vertex), vjust=-.5)

```

Clustering coefficient The extent to which neighbours of a node are also connected between them. Clustering coefficient is linked to the idea of **small-world networks**.

```{r}
# Clustering Coefficient using igraph
ccs <- igraph::transitivity(benguela.network, type='local')

ccs_df <- data.frame(vertex=as.numeric(V(benguela.network)), ccs)
ccs_df <- ccs_df[-which(is.nan(ccs)),]
ccs_df <- ccs_df[order(ccs_df$ccs, decreasing = TRUE),]
ccs_df$rank <- 1:dim(ccs_df)[1]

ggplot(ccs_df, aes(rank, ccs)) + geom_point(colour='navy', size=3) + theme_bw() +
  xlab('Rank') + ylab('Clustering Coefficient') + 
  geom_text(aes(label=vertex), vjust=-.5)
...
require(rnetcarto)
# Calculate modularity using the netcarto algorithm
modularity <- netcarto(benguela.AM)

print(modularity)

plot(benguela.network, vertex.color=rainbow(4), layout=layout_with_gem, edge.arrow.size = 0.2)

```

### 1.6.9 Nestedness

Nestedness allows us to quantify the extent to which the species composition (or composition interaction partners if thinking of a plant animal interaction network) of less speciose communities (or interacting partners of specialist species in a network) are proper subsets of more diverse communities (or of the set of interacting partners of generalist species if thinking about a bipartite interaction network). 
...
```{r warning=FALSE, echo=TRUE, error=FALSE, message=FALSE}
require(bipartite)
# Calculating nestedness using the NODF metric
plotweb(t(anemonef))

nested_anemone <- nest.smdm(t(anemonef))

print(nested_anemone)
```
